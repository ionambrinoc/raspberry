...
public class Book 
{
		...
		{ 
			...
			else
			{
				...
				else
				{
					...
					{	while (searcher.nextOrderInBook !=null 
							&& hasher(searcher.nextOrderInBook.idNumber) == hash) 
						{ searcher = searcher.nextOrderInBook; if (searcher==null) System.out.println("WTF"); }
	...
	public void removeOrder (int idNumber) throws Throwable //OK
	{
		...
		if (current!=null)
		{
		System.out.println(current.idNumber);
		if (current.buy==true) 
			{
			    if (BuyTree.getLimit(current.getPrice())!=null)
			    {
				...
				{
				if (SellTree.getLimit(current.getPrice())!=null)
				{	
	...
	public void modifyOrder(int idNumber, int vol, int price) throws Throwable //OK
	{
		...
		if (current.getPrice() != price)
		{
			...
			if (current.buy)
			{
				...
				if (to==null) { BuyTree.addLimit(price); to=BuyTree.getLimit(price); }
				to.addOrder(current);
			...
			else
				{
					...
					if (to==null) { SellTree.addLimit(price); to = SellTree.getLimit(price); }
					to.addOrder(current);
					current.modify(vol, price);
				}
